...
import { Crown, ArrowLeft, Loader2, CreditCard, Landmark, Zap, Building } from 'lucide-react';
...
export function ChangePlan({ onComplete, onBack }: ChangePlanProps) {
  ...
  // Prorated billing (days remaining in month, billing day = 1st)
  const today = new Date();
  const daysInMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
  const daysRemaining = daysInMonth - today.getDate() + 1;
  const seatDelta = (bronzeSeats - BASE_BRONZE) * plan.bronzePerSeat
    + (silverSeats - BASE_SILVER) * plan.silverPerSeat
    + (goldSeats - BASE_GOLD) * plan.goldPerSeat;
  const proratedAmount = Math.round((daysRemaining / daysInMonth) * seatDelta);

  const handleSelectPlan = (id: PlanId) => {
    setSelectedPlanId(id);
    if (!PLANS.find(p => p.id === id)!.annualAvailable) setIsAnnual(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0A0A0A] transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        <div className="mb-8 text-center">
          <h1 className="text-2xl sm:text-3xl font-bold text-[#0E2250] dark:text-white mb-2 transition-colors duration-300">Change Your Plan</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm transition-colors duration-300">
            Select a new plan and adjust your seat allocation. Changes take effect from the next billing cycle.
          </p>
        </div>
                ...
                {PLANS.map(p => (
                  ...
                  >
                    ...
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 transition-colors duration-200 ${
                      ...
                      {React.createElement(PLAN_ICONS[p.id as keyof typeof PLAN_ICONS], {
                        size: 16,
                        className: selectedPlanId === p.id ? 'text-white' : 'text-gray-500 dark:text-gray-400',
                      })}
                    </div>
            ...
            <div className="bg-white dark:bg-[#141414] rounded-2xl border border-gray-200 dark:border-[#2A2A2A] p-6 transition-colors duration-300">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setBronzeSeats(Math.max(0, bronzeSeats - 10))} className="w-9 h-9 rounded-lg border border-gray-200 dark:border-[#2A2A2A] flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] text-lg font-bold transition-colors duration-200">−</button>
                    <input type="number" min={0} value={bronzeSeats} onChange={e => setBronzeSeats(Math.max(0, parseInt(e.target.value) || 0))} className="flex-1 h-9 text-center rounded-lg border border-gray-200 dark:border-[#2A2A2A] bg-gray-50 dark:bg-[#1C1C1C] text-[#0E2250] dark:text-white text-sm font-semibold focus:outline-none focus:border-[#E35000] transition-colors duration-200" />
                    <button onClick={() => setBronzeSeats(bronzeSeats + 10)} className="w-9 h-9 rounded-lg border border-gray-200 dark:border-[#2A2A2A] flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] text-lg font-bold transition-colors duration-200">+</button>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5 transition-colors duration-300">Subtotal: LKR {bronzeTotal.toLocaleString()}/mo</p>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-gradient-to-br from-[#0E2250] to-[#1e3a8a] rounded-2xl p-6 text-white overflow-hidden relative">
              ...
              <div className="space-y-3 text-sm relative z-10">
                ...
                {proratedAmount > 0 && (
                  <div className="flex justify-between text-amber-300 border-b border-white/10 pb-3">
                    <span className="leading-tight">Prorated charge<br /><span className="text-[10px] text-amber-200/70">billed immediately</span></span>
                    <span className="font-medium">+LKR {proratedAmount.toLocaleString()}</span>
                  </div>
                )}
                {proratedAmount < 0 && (
                  <div className="flex justify-between text-emerald-300 border-b border-white/10 pb-3">
                    <span className="leading-tight">Credit to next invoice<br /><span className="text-[10px] text-emerald-200/70">no immediate refund</span></span>
                    <span className="font-medium">LKR {Math.abs(proratedAmount).toLocaleString()}</span>
                  </div>
                )}
