...
import { ArrowRight, Loader2, Smartphone, ShieldCheck, ArrowLeft } from 'lucide-react';
...
export function SignIn({ onSignIn, onNavigateToSignUp }: SignInProps) {
  ...
  const handleEmailSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!mobile) { toast.error('Please enter your registered mobile number'); return; }
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      setStep('otp');
  ...
  if (step === 'otp') {
    return (
      <AuthLayout
        ...
        subtitle={`We sent a 6-digit code to +94 ${mobile}`}
      >
        <div className="space-y-6">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="flex justify-between items-center pt-1">
              ...
              >
                <ArrowLeft size={14} className="mr-1" /> Change details
              </button>
              <button
                type="button"
      ...
      <div className="space-y-6">
        <form onSubmit={handleEmailSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="mobile" className="dark:text-gray-200 transition-colors duration-300">
              Registered Mobile <span className="text-red-500">*</span>
            </Label>
            <div className="relative flex">
              <span className="inline-flex items-center px-3 h-11 text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[#2A2A2A] border border-r-0 border-gray-200 dark:border-[#2A2A2A] rounded-l-md select-none">
                <Smartphone size={15} className="mr-1.5" />+94
              </span>
              <Input
                id="mobile"
                type="tel"
                placeholder="71 234 5678"
                className="flex-1 h-11 rounded-l-none bg-gray-50 dark:bg-[#1C1C1C] border-gray-200 dark:border-[#2A2A2A] focus:bg-white dark:focus:bg-[#141414] dark:text-white dark:placeholder-gray-500 transition-colors duration-300"
                value={mobile}
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/30 rounded-lg p-3 transition-colors duration-300">
            <p className="text-xs text-blue-800 dark:text-blue-300 text-center transition-colors duration-300">
              OTP will be sent to your registered mobile via SMS.
            </p>
          </div>
          ...
          >
            {isLoading ? (
              ...
              <>Send OTP via SMS <ArrowRight size={18} className="ml-2" /></>
            )}
          </Button>
        </form>
